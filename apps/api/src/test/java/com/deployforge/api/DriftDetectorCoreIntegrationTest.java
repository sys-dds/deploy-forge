...
class DriftDetectorCoreIntegrationTest extends DriftIntegrationTestSupport {
    @Autowired MockMvc mockMvc;

    @Test
    void matchingActualHasNoFindingAndMismatchUpdatesOneOpenFinding() throws Exception {
        ...
        assertThat(checkDrift(mockMvc, projectId, serviceId, envId).get("findings").size()).isZero();

        deploymentReport(mockMvc, projectId, targetId, null, "sha256:other", "RUNNING");
        JsonNode first = checkDrift(mockMvc, projectId, serviceId, envId);
        JsonNode second = checkDrift(mockMvc, projectId, serviceId, envId);
        assertThat(first.get("findings").get(0).get("driftType").asText()).isEqualTo("ARTIFACT_DRIFT");
        assertThat(listDrift(mockMvc, projectId, "OPEN").size()).isEqualTo(1);
        assertThat(second.get("findings").get(0).get("driftFindingId").asText()).isEqualTo(first.get("findings").get(0).get("driftFindingId").asText());
    }
}
